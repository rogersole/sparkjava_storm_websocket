...
/**
 * Separates initial (first time) deliveries off of a RabbitMQ queue from redeliveries (messages
 * that are being processed after initial processing failed for some reason).
 *
...
public class RedeliveryStreamSeparator implements MultiStreamSplitter {
    ...
    private static final List<String> streams                 = Collections.unmodifiableList(Arrays.asList(
