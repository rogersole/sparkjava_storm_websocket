...
/**
 * Simple configuration class to allow users to specify a set hosts to connect to for high
...
public class ConfigAvailableHosts implements Serializable {
    ...
    /**
     * 
     * @return Whether or not there are any configured high availability hosts in this config
     */
    ...
    public Address[] toAddresses() {
        ...
        for (final Entry<String, Integer> entry : hostsMap.entrySet()) {
            if (entry.getKey() != null) {
                addresses[i++] =
                                entry.getValue() == null ? new Address(entry.getKey()) : new Address(entry.getKey(),
                                                entry.getValue());
            }
        }
        return addresses;
